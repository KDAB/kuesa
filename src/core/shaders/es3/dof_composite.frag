...
{
    highp float param = texture(depthTexture, texCoord).x;
...
void main()
{
    ...
    highp vec4 dofTextureValue = texture(dofTexture, texCoord);
    highp vec4 sharpTextureValue = texture(textureSampler, texCoord);
