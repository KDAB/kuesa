...
layout(binding = auto) uniform sampler2D brdfLUT;
              ...
              const in float ambientOcclusion)
{
    ...
    if (light.type != TYPE_DIRECTIONAL) {
        ...
        vec3 sUnnormalized = vec3(light.position) - wPosition;
        ...
        if (sDotN > 0.0) {
            ...
            if (light.range > 0.0)
            {
                ...
                float d2OverR2 = d2/(light.range * light.range);
        ...
        if (light.type == TYPE_SPOT) {
            ...
            float cd = dot(-light.direction, s);
