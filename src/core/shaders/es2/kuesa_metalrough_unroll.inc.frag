...
FP vec3 pbrModel(const in Light light,
                 ...
                 const in FP float ambientOcclusion)
{
    ...
    if (light.type != TYPE_DIRECTIONAL) {
        ...
        FP vec3 sUnnormalized = vec3(light.position) - wPosition;
        ...
        if (sDotN > 0.0) {
            ...
            if (light.range > 0.0)
            {
                ...
                FP float d2OverR2 = d2/(light.range * light.range);
        ...
        if (light.type == TYPE_SPOT) {
            ...
            FP float cd = dot(-light.direction, s);
            FP float angularAttenuation = clamp(cd * light.lightAngleScale + light.lightAngleOffset, 0.0, 1.0);
    ...
    FP vec3 color = att * sDotN * light.intensity * light.color
